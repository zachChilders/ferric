...
impl LspServer {
    ...
    async fn run_and_publish(&self, uri: Url, version: i32, source: String) {
        ...
        let snapshot =
            tokio::task::spawn_blocking(move || cache.run_and_store(&uri_s, version, &source))
                .await
...
impl LanguageServer for LspServer {
    ...
    async fn completion(&self, params: CompletionParams) -> RpcResult<Option<CompletionResponse>> {
        ...
        let Some(snap) = self.snapshot_for(uri) else {
            return Ok(None);
        };
    ...
    async fn document_symbol(
        ...
        let Some(snap) = self.snapshot_for(&params.text_document.uri) else {
            return Ok(None);
        };
