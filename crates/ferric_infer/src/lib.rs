                    ...
                    Ty::Fn {
                            ...
                            arg_tys.iter().zip(param_tys.iter()).enumerate()
                        {
                                ...
                                || matches!(param_resolved, Ty::Var(_))
                            {
                                ...
                                // Route through `unify` so alias-vs-non-alias
                                // pairs surface as `AliasMismatch` rather
                                // than a generic Mismatch (M10 Task 6 Part C).
                                // Pass `(param, arg)` so the diagnostic reads
                                // "expected <param>, got <arg>"; use the call
                                // span to match the M10 fixture convention.
                                if !dedicated {
                            ...
                            match candidates.len() {
                                0 => {
                                    // Same alias-aware routing as above.
                                    if !dedicated {
                                        self.unify(param_ty, arg_ty, *span);
                                    }
                                }
